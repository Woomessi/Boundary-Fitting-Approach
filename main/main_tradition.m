...
%   the optimal uniformity indicator using the traditional method. 
...
% for the concave surface
%
% Sol = importdata("C:\projects\codes\opt\sol\concave\tradition\Sol.mat");
%
% for the convex surface
%
Sol = importdata("C:\projects\codes\opt\sol\convex\tradition\Sol.mat");
...
T_point_path_all = cell(size_path,1);
...
for i = 1:size_path-1
    ...
    z_ref_now = z_ref_now - offset;
    % the velocity of the spray gun along the current pass
    v_current = solution(2);
    % the spray distance of the whole intersection points
    h1 = solution(3);
    
    % Calculate the transformation matrix and the coating thickness 
    % resulting from a certain pass 
    [T_point_path, thickness] = singlePassLocalSlicing(z_ref_now, v_current, point, facet, size_point, a, h1, h, beta_1, beta_2, q_max, b);
...
solution = Sol(i,:);
offset = solution(4);
% calculate the Z coordinate of the last slice plane, it couldn't exceed
% the lowest vertex of the model
z_ref_now = z_min + offset;
v_current = solution(2);
h1 = solution(3);
[T_point_path, thickness] = singlePassLocalSlicing(z_ref_now, v_current, point, facet, size_point, a, h1, h, beta_1, beta_2, q_max, b);
sum_thickness = sum_thickness + thickness;
...
% drawFigure_T_point_path_all(facet, point, T_point_path_all, sum_thickness, 1, size_path)